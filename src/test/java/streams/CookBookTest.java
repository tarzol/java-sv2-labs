...
class CookBookTest {
    ...
    void listCakeNamesWithGivenIngredient() {
        List<String> names = cookBook.listCakeNamesWithGivenIngredient("dió");
        assertEquals(3, names.size());
        assertTrue(names.contains("Bejgli"));
        assertFalse(names.contains("Linzer"));

    }

    @Test
    void listCakeNamesWithMaxIngredients() {
        List<String> expected = cookBook.listCakeNamesWithMaxIngredients(5);
        assertEquals(3, expected.size());
        assertTrue(expected.contains("Linzer"));
        assertFalse(expected.contains("Mézeskalács"));
    }
}